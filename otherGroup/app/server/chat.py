...
class Chat:
	...
	def proses(self,data):
		...
		try:
			...
			if (command=='auth'):
				...
				logging.warning('GET: download file {}'.format(params[0]))
				return self.get_file(params)
			elif (command=='upload'):
				sessionid = j[1].strip()
				params = [x for x in j[2:]]
				logging.warning('UPLOAD: upload file {}'.format(params[0]))
	...
	def get_file(self, params=[]):
		try:
			...
			return {'status': 'OK', 'message': 'File diterima', 'file':filecontent}
	...
	def upload_file(self, params=[]):
		try:
			...
			fp.write(isifile)
			fp.close()
			return {'status': 'OK', 'message': 'File dikirim'}
		except Exception as e:
			return {'status': 'ERROR', 'message': f'{e}'}
